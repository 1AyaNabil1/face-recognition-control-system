    ...
    if st.button("🚀 Recognize Face"):
        with st.spinner("Processing..."):
            response = requests.post(
                "http://127.0.0.1:5000/api/recognize", json={"image": img_b64}
            )
        ...
        if response.status_code == 200:
            ...
            with col2:
                st.image(
                    annotated_image, caption="Detected Faces", use_container_width=True
                )
        else:
            st.error("Recognition Failed. Please try again.")
